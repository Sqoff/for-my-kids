...
* **페널티권 스와이프 카드**: `내가 가진 페널티권`(포인트 › 페널티 & 방어권)과 `현재 등록된 패널티권 목록`(패널티권 관리 모달)은 퀘스트 배너와 동일한 좌우 스와이프 카드 캐러셀(scroll-snap, 한 장씩, 넘길 때 가벼운 햅틱)로 표시.
  * 카드 정보: 강조 문구(보유자 / 사용 가능 / ⏳ 쿨타임) · 제목 · 설명 2줄 · 우측 아이콘 · `n | 전체` 카운터 · 하단 액션(내 보유: ✏️ 🗑️ `사용하기 ⚡` / 관리 모달: `+ 1장 복제` `✏️ 수정` `🗑️ 삭제`).
  * 배경: 엄마 보유 핑크 파스텔, 아빠 보유 스카이 파스텔, 쿨타임 앰버 파스텔 (네이비 테마는 틴트 다크 그라데이션). 좁은 모달 안에서는 컴팩트 변형(`is-compact`) 사용.
...
### [화면 11] 알림 센터 (Notification Center) — 탭 시 해당 화면 즉시 이동
* 알림 카드 탭 → 읽음 처리 + 알림 센터 닫힘 + 관련 화면으로 이동 (가벼운 햅틱). 카드 우하단에 이동 대상 힌트(`수락/방어하기 ›`, `할 일 보기 ›` 등) 표시.
* 라우팅 규칙 (`getNotificationRoute`, 제목/유형 키워드 기반):
  * 페널티권 사용 공격 → 포인트 › 페널티 & 방어권, 상단 **수락/방어 배너**로 스크롤 + 강조
  * 기프티콘 → 포인트 › 기프티콘 저금통 / 퀘스트·달성·완주·보상 설정 → 퀘스트 탭 / 쿠폰·기타 패널티 → 포인트 › 페널티 & 방어권
  * 할 일 관련(등록·완료·수정·돕기·마감·유예) → 홈, 메시지의 `[할 일 제목]`과 일치하는 카드로 스크롤 + **파란 테두리 깜빡임 강조만** (수정 모달은 열지 않음 — 할 일 상세가 곧 편집 폼이므로 알림에서 편집 화면이 뜨는 어색함 방지). 일치하는 할 일이 없으면 홈 할 일 목록으로 이동.
  * 스마트 넛지 등 시스템 알림 → 홈
