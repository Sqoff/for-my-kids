...
#### 2) 육퇴 보너스 룰렛 디자인 및 동적 셔플 명세
- **칸 구분선 명확화**: 부채꼴 슬롯마다 선명한 다크 아웃라인(`stroke="#0f172a" stroke-width="3.5"`)과 중앙 토스풍 메탈릭 핀(`GO`)을 배치하여 칸 간 구분을 시각적으로 확연하게 분리.
- **동일 포인트 분리 & 동적 셔플 (Non-adjacent Interleaving)**:
  - 동일한 포인트인 **200P 2칸이 서로 붙어있지 않도록** 원형 배치 상에서 최소 1칸 이상의 거리를 두도록 검증(`dist > 1 && dist < 5`).
  - 모달을 열 때마다 6개 슬롯의 위치가 무작위로 재배치(동적 셔플)되어 매번 신선하고 역동적인 인터랙션 제공.
- **6칸 구성 및 확률 명세 (수학적 완전 균등 6분할)**:
  - `대박 1,000P!` : 6칸 중 1칸 (16.7%)
  - `500P` : 6칸 중 1칸 (16.7%)
  - `200P` : 6칸 중 2칸 (33.3%, 마주보는 비인접 슬롯)
  - `20P` / `10P` : 6칸 중 각 1칸 (각 16.7%)
- **정밀 바늘 회전**: 상단 12시 고정 바늘 지침에 선택된 섹터가 완벽하게 일치하도록 각도 `(360 - theta) % 360`에 5바퀴(1800도)를 더하여 부드럽게 감속 회전(`cubic-bezier(0.15, 0.9, 0.25, 1)`).
...
### 8.4 토스(Toss) UI/UX 디자인 시스템 개편 및 탭 4 기본값 사양
...
3. **토스 저금통 스타일: 부부 기프티콘 저금통 서브탭 (토스어플3.jpg 벤치마킹)**:
   ...
   - 2대 리워드 타일(`쿠팡 2% 적립`, `육퇴 룰렛`) 및 원클릭 쿠팡 직결 검색 바 탑재.
