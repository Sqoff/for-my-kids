...
3. **토스 저금통 스타일: 부부 기프티콘 저금통 서브탭 (토스어플3.jpg 벤치마킹)**:
   ...
   - **생필품·장보기 쿠팡 직결 카드 정렬 완전 캡슐화**: 인풋 박스 내부에 `pr-20`과 `absolute right-1 top-1 bottom-1` 구조를 적용하여 모바일 전 기종 및 폴더블 화면에서도 파란색 `[검색 ➔]` 버튼이 카드 테두리를 벗어나지 않고 완벽하게 인풋 박스 내부에 캡슐화되어 정렬되도록 개선.
...
5. **토스 브랜드콘 스타일: 기프티콘 샵 모달 (토스어플1.jpg 벤치마킹)**:
...
### 8.5 뱅크샐러드(BankSalad) 스타일 '팩폭/위트 육아 잔소리' 백그라운드 지능형 알림 엔진 (Smart Nudge Push Notifications)

뱅크샐러드의 카드 지출 팩폭 멘트("니 꿈이 거지야?")의 유쾌하고 날카로운 인터랙션을 부부 육아에 접목:

```mermaid
flowchart TD
    Trigger[백그라운드 스마트 트리거 / 테스트 버튼] --> CheckSettings{설정 탭에서<br/>잔소리 알림 ON?}
    CheckSettings -- OFF --> Stop([알림 미발송])
    CheckSettings -- ON --> Analyze[부부 육아 데이터 실시간 분석]
    
    Analyze --> CondA{낮 시간대<br/>미완료 할 일 존재?}
    Analyze --> CondB{오늘 육아 집중 모드<br/>0분인가?}
    Analyze --> CondC{이번 달 기저귀/생필품<br/>지출 & 쿠팡 적립?}
    
    CondA -- 기저귀 할 일 미완료 --> DiaperMsg["👶 기저귀 떨어지면 손빨래 각이에요!<br/>뽀마 엉덩이가 울고 있어요😭"]
    CondA -- 장보기/생필품 미완료 --> ShopMsg["🛒 장바구니 생필품 주문 깜빡하셨나요?<br/>쿠팡 2% 저금통 적립 챙기고 배식 대란 방지!"]
    CondB -- 집중 0분 --> FocusMsg["📱 혹시 스마트폰이 아기인가요?!<br/>네 꿈이 폰중독 부모야?! 지금 당장 눈맞춤 ON!"]
    CondC -- 가계부 브리핑 --> ExpenseMsg["💸 이번 달 기저귀값 148,000원 쓰셨네요!<br/>뽀마키즈 2%로 치킨값 건져서 저금통에 쏙!"]
    
    DiaperMsg & ShopMsg & FocusMsg & ExpenseMsg --> Dispatch[로컬 푸시 알림 발송<br/>+ 앱 내 알림 센터 기록<br/>+ 화면 다이내믹 플로팅 토스트]
```

1. **지능형 분석 및 팩폭 멘트 카테고리**:
   - **기저귀 직격 팩폭**: 낮 시간 기저귀 관련 할 일이 미완료 상태일 때: *"👶 {이름}! 오늘 기저귀 사기로 한 거 안 잊었죠? 기저귀 떨어지면 손빨래 각이에요! 뽀마 엉덩이가 울고 있어요 😭"*
   - **생필품·장보기 팩폭**: 식자재/장보기 할 일 미완료 시: *"🛒 {이름}! 장바구니 생필품 주문 깜빡하셨나요? 쿠팡 2% 저금통 적립도 챙기고 오늘 저녁 배식 대란을 막으세요!"*
   - **노폰 집중 시간 0분 팩폭**: 낮 동안 집중 모드를 한 번도 켜지 않았을 때: *"📱 혹시 지금 스마트폰이 아기인가요?! 오늘 육아 집중 0분! 네 꿈이 폰중독 부모야?! 지금 당장 15분만 폰 내려놓고 눈맞춤 퀘스트 시작하세요!"*
   - **기저귀·생필품 가계부 위트 브리핑**: 지출과 적립 현황 리포트: *"💸 {이름}! 이번 달 기저귀·분유·생필품으로 약 148,000원 쓰셨네요! 그래도 뽀마키즈 쿠팡 2% 제휴로 {P}P나 건져서 저금통에 쏙! 선방 인정! 👍"*
2. **설정 탭 ON/OFF 제어 & 테스트 발송**:
   - `[부부 육아 설정 & 규칙]` 모달 내에 **스마트 잔소리 알림 ON/OFF 스위치 토글** 구비 (`STATE.smartNudgeEnabled`, 기본값 `true`, `localStorage` 영구 보존).
   - `[🧪 지금 잔소리 알림 받아보기 (테스트)]` 버튼을 제공하여 부모가 언제든 즉시 팩폭 알림을 체험하고 동작을 검증할 수 있음.
3. **시스템 알림 및 인앱 피드백 다중 발송**:
   - `@capacitor/local-notifications`를 통한 실제 안드로이드 시스템 상태바 푸시 알림 발송.
   - 앱 내 알림 센터(`STATE.users[...].notifications`)에 기록되어 벨(🔔) 뱃지 자동 갱신.
   - 화면 상단에 토스 스타일 다이내믹 플로팅 토스트 배너로 시각적 즉시 피드백 제공.

---

### 8.6 화이트톤(Light) / 네이비톤(Dark) 듀얼 테마 시스템 (추천이미지 2종 벤치마킹)

제공된 디자인 가이드 2종(`추천이미지2.png` 화이트톤, `추천이미지.png` 네이비톤)의 시각적 미학을 완벽하게 수용하여 사용자가 자유롭게 전환할 수 있는 테마 시스템 구축:

| 구분 | ☀️ 화이트톤 (White Tone - 라이트 모드) | 🌙 네이비톤 (Navy Tone - 다크 모드) |
|---|---|---|
| **벤치마킹 이미지** | `추천이미지2.png` | `추천이미지.png` |
| **디자인 감성** | 토스 감성의 화사하고 눈이 편안한 미니멀리즘 | 밤 육아 시 눈부심이 없는 깊고 신비로운 딥 네이비 |
| **전체 배경색** | `#F8FAFC` / `#F1F5F9` | `#0B132B` / `#070C18` |
| **카드 / 섹션 배경** | 순백색 `#FFFFFF` (소프트 섀도우 포함) | 딥 네이비 `#131D38` / `#16203D` |
| **테두리(Border)** | 연회색 `#E2E8F0` | 네이비 테두리 `#1E293B` |
| **기본 / 보조 텍스트** | `#0F172A` / `#64748B` | `#F8FAFC` / `#94A3B8` |
| **앱바 / 하단 네비게이션** | 순백색 `#FFFFFF` + 상/하단 은은한 라인 | `#0F172A` / `#131D38` |

1. **원클릭 테마 전환 (Instant Toggle)**:
   - 상단 모바일 앱바 우측 액션 영역에 **테마 전환 원터치 아이콘(☀️ / 🌙)** 제공.
   - 클릭 1번으로 앱 전반의 배경, 카드, 모달, 텍스트가 즉각 전환되며 진동 피드백(`Haptics.impact`) 및 알림 토스트 출력.
2. **설정 모달 내 테마 선택 카드 UI**:
   - `[부부 육아 설정 & 규칙]` 모달 내에 `[☀️ 화이트톤 (밝은 테마)]` vs `[🌙 네이비톤 (다크 테마)]` 세련된 카드 선택기 제공.
3. **영구 상태 보존**:
   - `localStorage`(`bboma_app_theme`)에 저장되어 앱을 재실행해도 사용자가 선택한 테마가 변함없이 유지됨.
